...
if ! which lb > /dev/null ; then
	...
	if [ ! -d live-build ]; then
		wget https://launchpad.net/debian/+archive/primary/+files/live-build_3.0~a16.orig.tar.gz
		tar zxvf live-build_3.0~a16.orig.tar.gz
		mv live-build-3.0~a16/ live-build/
	fi
