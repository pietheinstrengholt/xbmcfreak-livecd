...
EOF
	...
	chown -R $xbmcUser:$xbmcUser /etc/mt-daapd.conf
	...
	#wakeonlanconfig
	update-rc.d -f wakeonlanconfig defaults >/dev/null 2>&1 &

	#logitech dinovo edge
	sed -i "s/hiddev/hidraw/g" /lib/udev/rules.d/70-hid2hci.rules

	#apache2
	echo "XBMCLive" > /etc/hostname
...
VHOSTNAME=`cat /etc/xbmc/hostname`

#fix hostname error
if ! grep -i -q $VHOSTNAME /etc/hosts ; then
        sed '/127.0.1.1/D' -i /etc/hosts
        echo "127.0.1.1       $VHOSTNAME" >> /etc/hosts
fi
